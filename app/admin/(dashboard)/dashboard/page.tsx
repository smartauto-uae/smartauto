...
type SeoSummary = {
  ...
  noTitle: number; noDesc: number; noOgImage: number
}
type RecentPost = {
  id: string; title: string; slug: string
  status: 'published' | 'draft'; created_at: string
}
type DashboardData = {
  seo: SeoSummary; recentPosts: RecentPost[]; totalPosts: number
}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <p style={{ fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.15em', fontWeight: 600, color: accent ? GOLD3 : '#7a7264' }}>
          {label}
        </p>
        <div style={{
      ...
      <div>
        <p style={{ fontSize: 'clamp(1.4rem,2.5vw,1.9rem)', fontWeight: 700, lineHeight: 1, marginBottom: '0.25rem', color: accent ? GOLD : '#1a1814' }}>
          {value}
        </p>
        {sub && <p style={{ fontSize: '0.72rem', color: '#7a7264' }}>{sub}</p>}
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div style={{ height: 6, borderRadius: 999, background: '#ede8de', overflow: 'hidden' }}>
        <div style={{ height: '100%', borderRadius: 999, width: `${p}%`, background: color, transition: 'width 0.7s ease' }} />
      </div>
    </div>
  )
}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '2rem' }}>
        ...
        <button
          ...
          style={{
            ...
            color: '#7a7264', cursor: isPending ? 'not-allowed' : 'pointer',
            opacity: isPending ? 0.6 : 1, transition: 'border-color 150ms',
          }}
          aria-label="Refresh dashboard"
        >
          <RefreshCw size={13} style={{ animation: isPending ? 'spin 1s linear infinite' : 'none' }} aria-hidden="true" />
          Refresh
        </button>
      </div>
            ...
            <>
              ...
              {data.seo.incomplete > 0 && (
                <Link href="/admin/seo" style={{
                  ...
                  background: '#fef2f2', border: '1px solid #fca5a5', textDecoration: 'none',
                }}>
